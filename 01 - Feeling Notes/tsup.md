...
- **マルチフォーマット出力**: ESM、CommonJS、IIFE形式に対応（UMDはプラグイン経由）
...
## 💡 なぜ重要か
npmパッケージを公開する際、ESMとCommonJSの両方に対応する必要があるが、TypeScriptコンパイラ（tsc）は一度に一つの形式しか出力できない。tsupは一回のコマンドで両形式を生成し、さらに型定義ファイルのバンドルも行うため、シンプルなライブラリ公開に適したツールとなっている。

## 🔗 つながり
→ [[esbuild]], [[Typescriptビルドツール比較分析]], [[JavaScriptパッケージ公開ツール比較分析]], [[TypeScript二段階ビルドパターン]]
