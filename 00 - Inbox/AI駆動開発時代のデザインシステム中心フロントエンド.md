...
### ステップ3: GitHub Packagesへのプライベート公開

生成したUIライブラリーを2つのnpmパッケージとしてGitHub Packagesに公開します：

- **@company/ui-library**: 開発者が`npm install`で導入するUIコンポーネント
- **@company/ui-mcp**: `npx`で起動するMCPサーバー（AIがコンポーネント情報を理解）

社内利用のため、GitHub PATを使用したアクセス制御により、セキュアな配布を実現します。
